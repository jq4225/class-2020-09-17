...
```{r Scene 1}
en_fall_2020 <- read_excel("raw_data/class_enrollment_summary_by_term_9-17-2020.xlsx", skip = 3, col_names = TRUE)

en_fall_2020 <- en_fall_2020 %>%
  clean_names() %>%
  select(-instructor_full_name, -grad, -course_section_code, -non_degree, -x_reg, -vus, -employee, -withdraw, -total) %>%
  filter(u_grad >= 10) %>%
  drop_na()
